...
def predict_from_plot(plot: str):
    r = requests.post(API_PREDICT_PLOT, json={"plot": plot}, timeout=10)
    r.raise_for_status()
    data = r.json()
    return f"{data['label']} (conf={data['confidence']:.2f})"

def recommend_from_plot(plot: str, k: int = 5):
    r = requests.post(API_RECO_PLOT, json={"plot": plot, "k": k}, timeout=10)
    r.raise_for_status()
    data = r.json()
    ...
    lines = []
    for rec in recs:
        lines.append(f"- {rec['label']} | dist={rec['distance']:.3f} | {rec['plot'][:120]}...")
    return "\n".join(lines)