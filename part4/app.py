...
PLOT_LABELS_PATH = os.path.join(BASE_DIR, "weights/labels.json")
...
parser.add_argument("--model_path", type=str, default="weights/poster_classifier_resnet.pth", help="model path")
...
# ---- Load CLIP+Annoy assets (Part 4) ----

# Load multimodal assets for /discover_movies route
...
def home():
    return jsonify({
        "service": "movie-poster-api",
        "routes": [
            "/health",
            "/predict",
            "/batch_predict",
            "/validate_poster",
            "/batch_validate_poster",
            "/predict_plot",
            "/recommend_from_plot"
        ]
    })
...
def predict():
    ...
    with torch.no_grad():
        ...
        probs = F.softmax(outputs, dim=1)[0]    # (C,)
        pred_id = int(torch.argmax(probs).item())
...
def validate_poster():
    ...
    with torch.no_grad():
        features = model(tensor, return_features=True)

    score = dknn.compute_scores(features)
    return jsonify({"is a poster": bool((score < threshold).item())})
...
def batch_validate_poster():
    ...
    is_poster = (scores < threshold).tolist()
    return jsonify({"is a poster": is_poster})
